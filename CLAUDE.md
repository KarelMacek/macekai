...
- `app-backend`/`app-frontend` deploy together as **one Docker container per environment** to **Azure App Service**, via `.github/workflows/deploy_environments.yml`. Live and working for `dev`/`staging`/`prod`.
- **Merges to `main` go through a PR — no direct pushes to `main`, ever** (requested 2026-09-14, after a run of direct `git push origin main:main`-style promotions that shipped straight to prod with no review trail: "this has been too silent"). Feature/fix work still commits directly and pushes straight to `dev`/`staging` to exercise the deploy pipeline early — only the final `main`/prod promotion needs a PR, opened from whatever branch is ready to promote, reviewed, then merged normally. `main`'s GitHub Environment already has a required-reviewer rule on the *deploy job itself* (see `infra/README.md`) — that's a separate, additional gate, not a substitute for the PR.
...
- **Easy Auth via App Service's `auth_settings_v2`**, provider **Google only** (`google_v2` block) — explicit decision, no Microsoft/Entra fallback. `asistentka`'s original Entra-based setup was fully replaced same day it was built; the vestigial Entra app registration (`macekai-app-backend`) is unused. `common.hcl`'s `google_client_id` is filled in (Google Cloud project "MacekAI"); the client secret lives only at `~/.macekai/google_client_secret.txt` locally, never in the repo. Live and verified **end-to-end in a real browser** for `dev` (login → `/api/whoami/` recognizes the user) — `staging`/`prod` still need `web_app`/`key_vault` applied (same client works across all three, redirect URIs for all three were added up front). `tenant_id` in `common.hcl` is unrelated to login — it's required by the `key_vault` module for the Key Vault resource's own `tenant_id` field, keep it regardless of auth provider. The **Azure OpenAI** module is kept from `asistentka` too, model deployment gated off (see below).
  - **Claim-mapping gotcha**: Azure Easy Auth normalizes Google's OIDC claims into legacy WS-Identity claim types rather than passing through `sub` — the object id actually arrives as `http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier`, not `sub`/`oid`. `backend/auth/easy_auth.py`'s `_OID_TYPES` checks that claim first. Diagnosed by temporarily logging the raw decoded `X-MS-CLIENT-PRINCIPAL` claims at WARNING level while tailing `az webapp log tail` — see `infra/README.md`'s status log for the full story.
  - **Related fix, same investigation**: Azure's internal container warm-up probe hits with `Host: 169.254.x.x:8000` (link-local/APIPA, never internet-routable), which Django's `ALLOWED_HOSTS` rejected as `DisallowedHost`. `backend/auth/host_middleware.py` (`AzureInternalProbeHostMiddleware`, first in `MIDDLEWARE`) normalizes that specific range before Django's host check runs.
