...
- **Easy Auth** (Entra ID login via App Service's `auth_settings_v2`) and the **Azure OpenAI** module are kept from `asistentka`. `common.hcl`'s `tenant_id`/`easy_auth_client_id` are filled in (real Entra app registration `macekai-app-backend`); the client secret lives only at `~/.macekai/easy_auth_client_secret.txt` locally, never in the repo.
