...
def journey(db):
    journey = Journey.objects.create(
        slug="test-journey", is_active=True,
        simpleshop_product_id_cs="PROD1", simpleshop_product_id_en="PROD1-EN",
    )
...
def test_whoami_purchased_language_empty_without_diagnostics(user):
    ...
    assert resp.json()["current_diagnostics_id"] is None
...
def test_whoami_purchased_language_reflects_latest_diagnostics(journey, user):
    ...
    latest = open_diagnostics(email=user.email, journey=journey, source_order_id="B", language="en")
    client = APIClient()
    client.force_login(user)
    resp = client.get("/api/whoami/")
    assert resp.json()["purchased_language"] == "en"
    assert resp.json()["current_diagnostics_id"] == latest.id
...
def test_webhook_opens_diagnostics_on_matching_product(journey):
    ...
    assert diagnostics.language == "cs"


def test_webhook_opens_diagnostics_on_matching_en_product(journey):
    resp = APIClient().get(
        _webhook_url(), {"mail": "buyer@example.com", "id": "ORDER1", "id_product": "PROD1-EN"}
    )
    assert resp.status_code == 200
    diagnostics = Diagnostics.objects.get()
    assert diagnostics.journey_id == journey.id
