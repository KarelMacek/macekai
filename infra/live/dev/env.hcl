locals {
  ...
  web_app_sku  = "B1" # Free (F1) tier doesn't support container-based ("Web App for Containers") deploy, so B1 is the floor here
  postgres_sku = "B_Standard_B1ms"

  # image tag pushed by the deploy workflow on every push to the `dev` branch
  image_tag = "dev-latest"

  # Azure OpenAI model deployment — placeholder, verify current availability/quota
  # for this model+version in app_location+this subscription before `terragrunt apply`.
